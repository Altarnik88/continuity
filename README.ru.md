...
`npm start` (то же, что `npm run launch`) открывает панель на порту 43147 и сразу запускает память и оркестрацию. В Cursor или Grok `/continuity` делает главного агента **дирижёром**: он поднимает память, наполняет базу задач и распараллеливает 5–20 субагентов на анализ, разработку, слепую проверку, безопасность и ревью. С размера 10 назначается управляющий. `launch.mjs --once` гоняет тот же рой без UI до конца текущей волны. Принятие остаётся pending, пока вы сами не примете результат.
