...
Treat continuity as a navigation aid, never as the source of truth. Verify relevant claims against current sources, Git state, and the environment. A historical pass is not a current pass, and only the user may accept or reject a result.

Continuity requires Node.js 22+ and Git. It is local and self-contained: no other Skill, daemon, network service, model installation, or interview flow is required.
...
Review and edit the bundled v3 template so its goal and criteria reflect explicit user intent, then run:
...
A Coordinator may read ready sets and record packets, assignments, reports, verification, and releases. Continuity does not launch or require one. It also does not start processes, daemons, network clients, or interviews. Graphify is an optional navigation adapter and never a source of truth or acceptance authority.
