...
3. If you add a file that should ship in `npm pack` / release zips, update `DISTRIBUTABLE_FILES`, `package.json` `files`, `scripts/release-profiles.mjs`, and `scripts/test-validate-package.mjs` PKG-015 together. `COMPETITORS.md` and `COMPETITORS.ru.md` are required metadata and pack files; keep that pair in lockstep.
